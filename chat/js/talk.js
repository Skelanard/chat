function talk() {
    var isFound = new Boolean(false); //deklaruje boolean'a który będzie sprawdzał czy element został znaleziony
    ...
    Object.keys(hello).forEach(element => {
        if (user.toLowerCase().includes(element)) {
            ...
            zawierającym objekt*/
            isFound = !isFound; /*element został znaleziony, dla tego wartość isFound ma zmienić się na true.
    ...
    Object.keys(time).forEach(element => {
        if (user.toLowerCase().includes(element)) {
            console.log('znaleziono w tablicy timee')
